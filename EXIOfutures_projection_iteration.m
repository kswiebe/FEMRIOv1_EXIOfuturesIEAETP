...
for year = (endyear+1):finalyear
    ...
    % start iteration for the feedback from VA to FD
    % initialize to 1 for every year
    convergencecrit = 1;
    iter = 1;
    while convergencecrit > 0.03 % maximum change in VA at the country level
        
        if(dispon==1); disp('Final demand at macro level....'); end;
        % what we want is to have GDPTR = GDPTRtemp (= GDPTRshouldbe, which
        % is implied as soon as the first equality holds)
        if iter == 1 %set exogenous growth rate
            MacroData.GDPTRtemp(:,t) = MacroData.GDPTRshouldbe(:,t-1) .* (1+ MacroData.GDPgrowth(:,t));
        else
            MacroData.GDPTRtemp(:,t) = MacroData.GDPTR(:,t); %from previous iteration
        end
        preiterationGDPTR = MacroData.GDPTRtemp(:,t);
        ...
        if(dispon==1); disp('Final demand at industry level....'); end;
        % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
%        disp(['sum(sum(natFDnew)) 2014 = ',num2str(sum(sum(natFDnew))/1000000000)])
        ...
        if usePADS == 1 && changeConsStructure == 0
            ...
            if(dispon==1); disp(['sum(sum(natFDnew)) after PADS = ',num2str(sum(sum(natFDnew))/1000000000)]); end;
        ...
        for reg=1:nreg
            ...
            natFDnew(:,(reg-1)*nfd+2) = SUT.natFD(:,(reg-1)*nfd+2) * (MacroData.NPSH(reg,t)/MacroData.NPSH(reg,t-1));
            natFDnew(:,(reg-1)*nfd+3) = SUT.natFD(:,(reg-1)*nfd+3) * (MacroData.GOVE(reg,t)/MacroData.GOVE(reg,t-1));
        ...
        if(dispon==1); disp(['sum(sum(natFDnew)) after energy = ',num2str(sum(sum(natFDnew))/1000000000)]); end;
        ...
        for reg=1:nreg
            natFDnew(:,(reg-1)*nfd+4) = SUT.natFD(:,(reg-1)*nfd+4) * (MacroData.GFCF(reg,t)/MacroData.GFCF(reg,t-1));
        end %for reg FDnew
        ...
        if(dispon==1); disp(['sum(sum(natFDnew)) after electricity = ',num2str(sum(sum(natFDnew))/1000000000)]); end;
        %disp(['min(min(natFDnew)) after electricity = ',num2str(min(min(natFDnew)))])
        ...
        if(dispon==1); disp('Changing input coefficients....'); end;
        % initialization with last years value
        % 1) default coefficient change: no change 
        natUSEcoefBnew = SUT.natUSEcoefB;
        ...
        if sum(IEAEXIO.EVshareincrease(:,t)) > 0
            ...
            % rescale as inputs are replaced by other inputs
...
%         disp(['min(min(natUSEcoefBnew) = ',num2str(min(min(natUSEcoefBnew)))])
        ...
        % change trade shares according to relative change in the share in
        % global GDP globalGDPtemp and sum(Macrodata.GDPTR(:,t-1))
        if iter == 1 % only change trade shares once to avoid divergence
            share_t = MacroData.GDPTRtemp(:,t) ./ repmat(globalGDPtemp,nreg,1);
            share_tminus1 = MacroData.GDPTR(:,t-1) ./ repmat(sum(MacroData.GDPTR(:,t-1)),nreg,1);
            tradeimportance = 0.5*ones(nreg,1);
            %tradeimportance([35 45]) = [0.8 0.8];
           % tradeimportance([35 45 47 49]) = [0.8 0.7 0.8 0.7]; %enhance positive effect 43  47
           % tradeimportance([30 43 29 38]) = [0.6 0.6 0.6 0.6]; %enhance positive effect 43  47
%             if year > 2020
%                 tradeimportance([30 43 ]) = [0.7 0.5];% 0.8 0.8]; %enhance positive effect 43  47
%             end
%             if year > 2023
%                 tradeimportance([29 38]) = [0.6 0.7]; %enhance negative effect
%             end
%             if year > 2027
%                 tradeimportance([29]) = [0.6]; %enhance negative effect
%             end
            %last working was year > 2025 29 = 0.7, and 49 wasn't altered
            share_gr(:,t) = tradeimportance.*(share_t./share_tminus1-1)+1; %% using a factor tradeimportance to reflect that exports do not fully follow GDP growth

            for c = 1:nreg
                c1 = (c-1)*nprod+1;
                c2 = c*nprod;
                SUT_t.ImpShareBilateral(c1:c2,:) = share_gr(c,t) * SUT.ImpShareBilateral(c1:c2,:);
                SUT_t.FDimpShareBilateral(c1:c2,:) = share_gr(c,t) * SUT.FDimpShareBilateral(c1:c2,:);
            end
            SUT_t.ImpShareBilateral(isnan(SUT_t.ImpShareBilateral))=0;
            SUT_t.ImpShareBilateral(isinf(SUT_t.ImpShareBilateral))=0;
            SUT_t.FDimpShareBilateral(isnan(SUT_t.FDimpShareBilateral))=0;
            SUT_t.FDimpShareBilateral(isinf(SUT_t.FDimpShareBilateral))=0;
            % rescale to 1 for all product groups
            for p = 1:nprod
                SUT_t.ImpShareBilateral(p:nprod:end,:) =  SUT_t.ImpShareBilateral(p:nprod:end,:) ./ repmat(sum(SUT_t.ImpShareBilateral(p:nprod:end,:)),nreg,1);
                SUT_t.FDimpShareBilateral(p:nprod:end,:) =  SUT_t.FDimpShareBilateral(p:nprod:end,:) ./ repmat(sum(SUT_t.FDimpShareBilateral(p:nprod:end,:)),nreg,1);
            end
            SUT_t.ImpShareBilateral(isnan(SUT_t.ImpShareBilateral))=0;
            SUT_t.ImpShareBilateral(isinf(SUT_t.ImpShareBilateral))=0;
            SUT_t.FDimpShareBilateral(isnan(SUT_t.FDimpShareBilateral))=0;
...
%         disp(['max(sum(MRUSEcoefBnew) = ',num2str(max(sum(MRUSEcoefBnew)))])
    ...
    %     % set extraterritorial organisations to zero before scaling and after
     ...
     %   disp(['sum(sum(MRSUPcoefDnew) = ',num2str(sum(sum(MRSUPcoefDnew)))])
        ...
        if circulareconomy == 1
           if(dispon==1); disp(['sum(sum(MRSUPcoefDnew) = ',num2str(sum(sum(MRSUPcoefDnew)))]); end;
           run('CircularEconomy_MarketShares.m');
           if(dispon==1); disp(['after circular economy sum(sum(MRSUPcoefDnew) = ',num2str(sum(sum(MRSUPcoefDnew)))]); end;
        end % circulareconomy
    ...
    %[Lindprod,Lprodprod] = Leontief_TotalRequirementsMatrix(SUThist.MRUSEcoefB,MRSUPcoefDnew,nreg,nprod,nind);
        ...
        if(dispon==1); disp('SUT table....'); end;
    ...
    %     plot(1:(nreg*nind),g,'go',1:(nreg*nind),(sum(MRUSE,1)+sum(MRVA(relVA,:))),'r*')
        ...
        if(dispon==1); disp('Value added....'); end;
        ...
        % convergence
        difftot = (MacroData.GDPTR(:,t) ./ preiterationGDPTR) - 1;
        diff = abs( (MacroData.GDPTR(:,t) ./ preiterationGDPTR) - 1);
        couconv = find(diff == max(diff));
        convergencecrit = max(diff);
        disp(['year ',num2str(year),' iter ',num2str(iter),': convergencecrit = ',num2str(difftot(couconv)),' cou ',num2str(couconv)]);
        iter = iter + 1;
    end %while convergencecrit > 0.01 % maximum change in VA at the country level
    if year==finalyear
        figure()
    ...
    %----------------------------------------------------------------------
    if(dispon==1); disp('change stressor intensities....'); end;
